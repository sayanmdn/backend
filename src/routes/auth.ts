...
// GET callback route for WorkOS redirect
router.get("/sso/callback", async (req: Request, res: Response) => {
  const frontendUrl = 'https://sayantanmishra.com';
  ...
  try {
    ...
    // Handle OAuth errors
    if (error) {
      return res.redirect(`${frontendUrl}/sso/error?error=${encodeURIComponent(error as string)}`);
    }

    if (!code) {
      return res.redirect(`${frontendUrl}/sso/error?error=missing_code`);
    }

    const authResult = await workosService.authenticateWithCode(code as string);

    if (!authResult.success) {
      return res.redirect(`${frontendUrl}/sso/error?error=${encodeURIComponent(authResult.error)}`);
    }
    ...
    const successUrl = `${frontendUrl}/sso/success?token=${encodeURIComponent(token)}&user=${encodeURIComponent(JSON.stringify({
      id: user._id,
      name: user.name,
      email: user.email,
      role: user.role || "USER",
    ...
    res.redirect(`${frontendUrl}/sso/error?error=callback_error`);
  }
});

// Keep POST callback for API-based flows
